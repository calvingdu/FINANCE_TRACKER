...
import csv
from backend.src.helper.schema.accounts_schema import bmo_cc_schema as schema
    ...
    try:
        ...
        with open(file_path+file_name, 'r') as csv_file:
            ...
            rows = [
                ...
                and not ('Item #Card #Transaction DatePosting DateTransaction AmountDescription' in ''.join(row))
                ]
...
def transform_data(df, account_name):
    ...
    df.drop(['bank_card', 'index', 'posting_date'], axis = 1, inplace=True)
